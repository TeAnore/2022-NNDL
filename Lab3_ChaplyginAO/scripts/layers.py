...
def softmax_loss(x, y):
    ...
    expx = np.exp(x)
    prob = expx[np.arange(x.shape[0]), y] / np.sum(expx, axis=1)

    loss = np.mean( (-1) * np.log(prob) )

    dexpx = np.zeros_like(expx)
    dexpx += ( 1 / np.sum(expx, axis=1) ).reshape(x.shape[0],1) 
    dexpx[np.arange(x.shape[0]), y] += (-1) / expx[np.arange(x.shape[0]), y]
    dx = dexpx * expx / x.shape[0]
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        xhat_test = (x-running_mean) / np.sqrt(running_var + eps)
        out = gamma * xhat_test + beta
...
def batchnorm_backward(dout, cache):
    ...
    dxhat = dout * gamma
    dvar  = np.sum(dxhat * x_centered * (-0.5) * inv_stdev**3, axis=0)
    dmean = np.sum(dxhat * (-1) * inv_stdev, axis=0)

    dx = (dxhat * inv_stdev) + (dvar * 2 * x_centered / N) + (dmean / N)
